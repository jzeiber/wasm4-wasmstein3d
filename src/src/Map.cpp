...
{
	// only need to *2 when map data size is reduced (only horizontal data in map)
	#ifdef RANDOMMAP
	const int32_t offset=((z*MAP_SIZE+x)*2);
	#else
	const int32_t offset=((z*MAP_SIZE+x)*2)+(int32_t(currentLevel)*MAP_SIZE*MAP_SIZE*4);
	#endif
	if(orientation==MapRead_Horizontal)
	{
		memcpy(buffer,&MapData_data[offset],length*2);
	}
	else
	{
		int32_t buffpos=0;
		for(int32_t i=0; i<length; i++,buffpos+=2)
		{
			memcpy(&buffer[buffpos],&MapData_data[offset+(i*MAP_SIZE*2)],2);
		}
	}
	return;

#ifdef FXDATA_STREAMING
// // default map data has horizontal data, then vertical data for each map - we don't need both, so we can reduce size by half
...
void Map::updateBufferPosition(int8_t newX, int8_t newZ)
{
	...
	|| newZ <= bufferZ - MAP_BUFFER_SIZE || newZ >= bufferZ + MAP_BUFFER_SIZE)
	{
		bufferX = newX;
		bufferZ = newZ;
		//WARNING("Updating entire buffer at %d %d\n%d %d %d %d", bufferX, bufferZ, engine.streamBuffer[0], engine.streamBuffer[1], m_mapBuffer[0], m_mapBuffer[1]);
		updateEntireBuffer();
		return;
	}
	...
	if(door)
	{
		...
		else
		{
			if ((door->type == DoorType_Locked1Horizontal || door->type == DoorType_Locked1Vertical) && !engine.player.inventory.hasKey1)
			{
				if (isPlayer)
				{
					if (!engine.player.blinkKeyTimer1)
					{
						Platform.playSound(NOWAYSND);
					}
					engine.player.blinkKeyTimer1 = TARGET_FRAMERATE;
				}
				return;
			}
			if ((door->type == DoorType_Locked2Horizontal || door->type == DoorType_Locked2Vertical) && !engine.player.inventory.hasKey2)
			{
				if (isPlayer)
				{
					if (!engine.player.blinkKeyTimer2)
					{
						Platform.playSound(NOWAYSND);
					}
					engine.player.blinkKeyTimer2 = TARGET_FRAMERATE;
				}
				return;
			}
...
bool Map::isSeen(int8_t cellX, int8_t cellZ)
{
	if(cellX<0 || cellX>=MAP_SIZE || cellX<0 || cellZ>=MAP_SIZE)
	{
		return false;
	}
	const int32_t bytepos=((cellZ*MAP_SIZE)+cellX)/8;
	const int32_t bitpos=((cellZ*MAP_SIZE)+cellX)%8;
	return m_cellseen[bytepos] & (0xb00000001 << (7-bitpos));
}

void Map::setSeen(int8_t cellX, int8_t cellZ)
{
	const int32_t bytepos=((cellZ*MAP_SIZE)+cellX)/8;
	const int32_t bitpos=((cellZ*MAP_SIZE)+cellX)%8;
	m_cellseen[bytepos]|=(0b00000001 << (7-bitpos));
}

void Map::clearSeen()
{
	memset(m_cellseen,0,(MAP_SIZE*MAP_SIZE/8)+1);
}
