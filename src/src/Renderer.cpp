...
void Renderer::drawSprite2D(uint8_t id, int16_t x, int16_t y)
{
	...
	for (int8_t i = 0; i < frameWidth; i++)
	{
		...
		//memcpy(buffer,&Data_uiSprite_data[spriteAddress],frameHeight);
		diskRead2BPP(buffer, &Data_uiSprite_data_2bpp[0], spriteAddress, frameHeight);
		uint8_t v = 0;
		...
		for (int8_t j = frameHeight - 1; j >= 0; j--)
		{
			...
			{
			case 1:
				drawPixel(i + x, y + j, PALETTE_WHITE);
				break;
			case 2:
				drawPixel(i + x, y + j, PALETTE_BLACK);
				break;
			}
		}
...
void Renderer::drawWeapon()
{
	...
	case WeaponType_ChainGun:
		...
		sprite = (uint8_t *)Data_chaingunSprite_data_2bpp;
	...
	for(int8_t i = 0; i < frameWidth; i++)
	{
		...
		diskRead2BPP(buffer, sprite, offset, frameHeight);
		uint8_t v = 0;
		...
		for(int8_t j = frameHeight - 1; j >= 0; j--)
		{
			...
			{
			case 1:
				drawPixel(i + x, DISPLAYHEIGHT - frameHeight + j, PALETTE_WHITE);
				break;
			case 2:
				drawPixel(i + x, DISPLAYHEIGHT - frameHeight + j, PALETTE_BLACK);
				break;
			}
		}
...
void Renderer::drawFrame()
{
	...
	for(int8_t n = 0; n < MAX_ACTIVE_ITEMS; n++)
	{
		if(engine.map.items[n].type != 0)
		{
			...
			queueSprite((SpriteFrame*) &Data_itemSprites_frames[(engine.map.items[n].type - Tile_FirstItem)], (uint8_t *)&Data_itemSprites_data_2bpp[0], x, z);
		}
	}
	...
	*DRAW_COLORS=(PALETTE_DARKGREY << 4) | PALETTE_DARKGREY;
	...
	for(int y=(engine.player.z/CELL_SIZE)-1; y<(engine.player.z/CELL_SIZE)+2; y++)
	{
		for(int x=(engine.player.x/CELL_SIZE)-1; x<(engine.player.x/CELL_SIZE)+2; x++)
		{
			engine.map.setSeen(x,y);
		}
	}

	if(engine.difficulty==Difficulty_Hard)
	{
		drawString("NO MAP",(DISPLAYWIDTH-(6*4))/2,DISPLAYHEIGHT+25,PALETTE_BLACK);
		drawString("FOR YOU",(DISPLAYWIDTH-(7*4))/2, DISPLAYHEIGHT+35,PALETTE_BLACK);
	}
	else
	{
		for(int y=0; y<MAP_SIZE; y++)
		{
			...
			{
				if(engine.difficulty<Difficulty_Hard && (engine.difficulty==Difficulty_Baby || engine.map.isSeen(x,y)))
				{
					const uint8_t tile=MapData_data[(y*MAP_SIZE*2) + x*2];
					if((tile>=Tile_FirstWall && tile<=Tile_LastWall) || tile==Tile_SecretPushWall)
					{
						*DRAW_COLORS=PALETTE_WHITE;
						line(mapoffsetx+x,y+DISPLAYHEIGHT,mapoffsetx+x,y+DISPLAYHEIGHT);
					}
					...
					if(x==(engine.player.x/CELL_SIZE) && y==(engine.player.z/CELL_SIZE) && (engine.frameCount%12)>2)
					{
	...
	// draw compass
	{
		constexpr uint8_t degrees45=DEGREES_90/2;
		int32_t centerx=DISPLAYWIDTH-((DISPLAYWIDTH-MAP_SIZE)/4);
		int32_t centery=SCREEN_SIZE-((SCREEN_SIZE-DISPLAYHEIGHT-6)/2);
	
		//*DRAW_COLORS=PALETTE_WHITE<<4;
		//oval(centerx-10,centery-10,20,20);

		drawGlyph('N'-FIRST_FONT_GLYPH,centerx-1,centery-16,PALETTE_LIGHTGREY);
		drawGlyph('S'-FIRST_FONT_GLYPH,centerx-1,centery+12,PALETTE_LIGHTGREY);
		drawGlyph('W'-FIRST_FONT_GLYPH,centerx-15,centery-2,PALETTE_LIGHTGREY);
	...
	// debug - draw graphic for player status
	/*
	UI_BJFace,
	UI_BJFace_Dead,
	UI_BJFace_Baby,
	...
	Data_uiSprite_frames
	*/

	{
		static uint8_t lasthp=0;
		static uint8_t hitframes=0;
		const int xoffset=(((DISPLAYWIDTH-MAP_SIZE)/2)-32)/2;
		
		*DRAW_COLORS=(PALETTE_WHITE << 4) | PALETTE_WHITE;
		rect(xoffset,DISPLAYHEIGHT+15,32,32);

		if(engine.player.hp<lasthp)
		{
			hitframes=12;
		}
		...
		{
			hitframes--;
			drawSprite2D(UI_BJFace_Easy,xoffset+((32-Data_uiSprite_frames[UI_BJFace_Easy].width)/2),DISPLAYHEIGHT+16);
		}
		else if(engine.player.hp>70)
		{	
			drawSprite2D(UI_BJFace_Medium,xoffset+((32-Data_uiSprite_frames[UI_BJFace_Medium].width)/2),DISPLAYHEIGHT+16);
		}
		else if(engine.player.hp>40)
		{
			drawSprite2D(UI_BJFace,xoffset+((32-Data_uiSprite_frames[UI_BJFace].width)/2),DISPLAYHEIGHT+16);
		}
		else
		{
			drawSprite2D(UI_BJFace_Dead,xoffset+((32-Data_uiSprite_frames[UI_BJFace_Dead].width)/2),DISPLAYHEIGHT+16);
		}
		lasthp=engine.player.hp;
	}
...
void Renderer::drawHUD()
{
	...
	constexpr int offsety = statusBarHeight;

	drawBox(0, offsety + DISPLAYHEIGHT - statusBarHeight, 48, statusBarHeight, PALETTE_BLACK);
	drawBox(DISPLAYWIDTH - 48, offsety + DISPLAYHEIGHT - statusBarHeight, 48, statusBarHeight, PALETTE_BLACK);

	drawBox(DISPLAYWIDTH - 48 + 1, offsety + DISPLAYHEIGHT - statusBarHeight + 1, 7, statusBarHeight - 1, PALETTE_WHITE);
	if (engine.player.inventory.hasKey1 || (engine.player.blinkKeyTimer1 && (engine.frameCount & 0x4)))
	{
		drawSprite2D(UI_Key1, DISPLAYWIDTH - 48 + 1, offsety + DISPLAYHEIGHT - statusBarHeight + 1);
	}

	// No need for key 2 in shareware
	drawBox(DISPLAYWIDTH - 48 + 9, offsety + DISPLAYHEIGHT - statusBarHeight + 1, 7, statusBarHeight - 1, PALETTE_WHITE);
	if (engine.player.inventory.hasKey2 || (engine.player.blinkKeyTimer2 && (engine.frameCount & 0x4)))
	{
		drawSprite2D(UI_Key2, DISPLAYWIDTH - 48 + 9, offsety + DISPLAYHEIGHT - statusBarHeight + 1);
	}

	drawBox(26, offsety + DISPLAYHEIGHT - statusBarHeight + 1, 1, statusBarHeight - 1, PALETTE_WHITE);

	int textLine = offsety + DISPLAYHEIGHT - FONT_HEIGHT * 2 - 1;
	int numberLine = offsety + DISPLAYHEIGHT - FONT_HEIGHT;

	drawString(PSTR("HEALTH"), 1, textLine, PALETTE_WHITE);
	drawGlyph('%' - FIRST_FONT_GLYPH, 18, numberLine, PALETTE_WHITE);
	drawInt(engine.player.hp, 14, numberLine, PALETTE_WHITE);
	drawString(PSTR("AMMO"), 30, textLine, PALETTE_WHITE);
	drawInt(engine.player.weapon.ammo, 38, numberLine, PALETTE_WHITE);

	drawString(PSTR("SCORE"), DISPLAYWIDTH - 30, textLine, PALETTE_WHITE);
	drawLong(engine.player.score, DISPLAYWIDTH - FONT_WIDTH - 1, numberLine, PALETTE_WHITE);
...
void Renderer::drawCell(int8_t cellX, int8_t cellZ)
{
	...
	{
		queueSprite((SpriteFrame*) &Data_decorations_frames[tile - Tile_FirstDecoration], (uint8_t *)&Data_decorations_data_2bpp[0], worldX + CELL_SIZE / 2, worldZ + CELL_SIZE / 2);
		return;
	}
	if(tile >= Tile_FirstBlockingDecoration && tile <= Tile_LastBlockingDecoration)
	{
		queueSprite((SpriteFrame*) &Data_blockingDecorations_frames[tile - Tile_FirstBlockingDecoration], (uint8_t *)&Data_blockingDecorations_data_2bpp[0], worldX + CELL_SIZE / 2, worldZ + CELL_SIZE / 2);
		return;
	}
	if (tile >= Tile_FirstItem && tile <= Tile_LastItem)
	{
		queueSprite((SpriteFrame*)&Data_itemSprites_frames[(tile - Tile_FirstItem)], (uint8_t *)&Data_itemSprites_data_2bpp[0], worldX + CELL_SIZE / 2, worldZ + CELL_SIZE / 2);
	}
...
/*inline*/ void Renderer::drawStrip(int16_t x, int16_t w, int8_t u, uint8_t textureId)
{
...
#endif
	{
		if(y >= 0 && y < DISPLAYHEIGHT)
		{
			...
			{
			case 0:
				drawPixel(x,y,PALETTE_LIGHTGREY);
				break;
			case 1:
...
void Renderer::drawWall(int16_t _x1, int16_t _z1, int16_t _x2, int16_t _z2, uint8_t textureId, int8_t _u1, int8_t _u2)
{
	...
	// clamp to the visible portion of the screen
	int16_t firstx = wasm4::max(sx1, (int16_t)0);
	int16_t lastx = wasm4::min(sx2, (int16_t)(DISPLAYWIDTH-1));
	if (lastx < firstx)
		return;
	...
	for (int x=sx1; x<=sx2; x++)
	{
		...
		{        
			...
			if (u != lastU)
			{
				...
				diskRead2BPP(engine.streamBuffer, &Data_wallTextures_data_2bpp[0], u * TEXTURE_STRIDE + textureId * (TEXTURE_STRIDE * TEXTURE_SIZE), TEXTURE_SIZE);
				lastU = u;
			}
...
void Renderer::queueSprite(SpriteFrame* frame, uint8_t* sprite, int16_t _x, int16_t _z)
{
#if 1
	...
	for(int n = 0; n < RENDER_QUEUE_CAPACITY; n++)
	{
		...
		if(renderQueue[n].sprite == nullptr)
		{
			newItem = n;
			break;
		}
	...
	renderQueue[newItem].sprite = sprite;
...
void Renderer::drawQueuedSprite(uint8_t id)
{
	...
	uint8_t outlineColour = PALETTE_BLACK;
	int8_t firstV = 0;
	...
	for (int x = sx1; x <= sx2; x++)
	{
		...
		{        
			...
			//memcpy(buffer,&renderQueue[id].sprite[renderQueue[id].frame->offset + frameHeight * u],frameHeight);
			diskRead2BPP(buffer, renderQueue[id].sprite, renderQueue[id].frame->offset + frameHeight * u, frameHeight);
...
void Renderer::drawGlyph(char glyph, uint8_t x, uint8_t y, uint8_t colour)
{
	...
	for(int i = 0; i < FONT_WIDTH; i++)
	{
		for(int j = 0; j < FONT_HEIGHT; j++)
		{
			...
			drawPixel(x + i, y + j, col ? 0 : colour);
			readMask <<= 1;
			if(readMask == 0)
			{
				readMask = 1;
...
void Renderer::drawLevelLoadScreen()
{
	...
	drawString(PSTR("GET PSYCHED!"), offsetx + 44, offsety + 5, PALETTE_DARKGREY);
	drawString(PSTR("FLOOR:"), offsetx + 51, offsety + 53, PALETTE_DARKGREY);
	drawInt(engine.map.currentLevel + 1, offsetx + 80, offsety + 53, PALETTE_DARKGREY);
	drawSprite2D(UI_BJFace, offsetx + 36, offsety + 16);
	drawGlyph('X' - FIRST_FONT_GLYPH, offsetx + 70, offsety + 32, PALETTE_DARKGREY);
	drawInt(engine.player.lives, offsetx + 82, offsety + 32, PALETTE_DARKGREY);
}

// void Renderer::drawBackground(uint24_t address)
// {
// 	//trace("Renderer::drawBackground TODO - replace this function");
// #ifdef _WIN32
...
void Renderer::drawBackground(uint8_t *data, const int32_t offsety)
{
	const int offsetx=(DISPLAYWIDTH/2)-64;
	int index=0;
	for(int y=0; y<64; y+=8)
...
void Renderer::fadeScreen(int8_t amount)
{
	...
	for (int y = 0; y < DISPLAYHEIGHT; y += 8)
	{
		for (int x = 0; x < DISPLAYWIDTH; x += 2)
		{
			for (int j = 0; j < 8; j++)
			{
				if (!(fadeMask1 & (1 << j)))
				{
					drawPixel(x, y + j, PALETTE_BLACK);
				}
				if (!(fadeMask2 & (1 << j)))
				{
					drawPixel(x + 1, y + j, PALETTE_BLACK);
				}
			}
		}
	}
	...
	// 	*ptr++ &= fadeMask2;
	// }
	for (int y = 0; y < SCREEN_SIZE; y += 8)
	{
		for (int x = 0; x < SCREEN_SIZE; x += 2)
		{
			for (int j = 0; j < 8; j++)
			{
				if (!(fadeMask1 & (1 << j)))
