...
{
	seed=(engine.frameCount << 16 | engine.frameCount) ^ (getRandomNumber16() << 16 | getRandomNumber16());
	map.currentLevel = 0;
	player.lives = 3;
	player.score = 0;
	player.hp = 0;
...
#ifdef RANDOMMAP
	generateRandomMap(MapData_data,seed,map.currentLevel);
#endif
	...
	case GameState_EnterNextLevel:
		{
			...
			*/
			...
			else if(map.currentLevel==9)	// shouldn't ever get here, but restart if so
			{
				map.currentLevel=0;
			}
			generateRandomMap(MapData_data,seed,map.currentLevel);
		#else
			// non random map - we only have 1 premade level so keep using that level
			map.currentLevel=0;
		#endif
			startLevel(false);
			save.saveStateToActiveSlot();
